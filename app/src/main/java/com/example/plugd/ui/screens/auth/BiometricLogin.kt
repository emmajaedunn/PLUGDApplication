...
// 🔹 Find the host Activity from any Compose context
...
fun BiometricLogin(
    ...
    TextButton(
        ...
        // optional: match Google button colour style
...
fun BiometricLogin(
    ...
    val systemSupportsBiometric = remember {
        canUseBiometric(context)
    }

    Button(
        onClick = {
            // basic checks before we even try
            if (!canAuthenticate || !systemSupportsBiometric) {
                Toast
                    .makeText(
                        context,
